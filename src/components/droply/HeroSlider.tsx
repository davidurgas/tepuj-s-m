...
const SLIDES: Slide[] = [
  {
    ...
    ctas: [
      { label: "Objaviť produkty", to: "/produkty", primary: true },
  ...
  },
  {
    ...
    ctas: [
      { label: "Chcem zľavu", to: "/balicky", primary: true },
      { label: "Pozrieť produkty", to: "/produkty" },
    ],
  },
  {
    kind: "image",
    ...
    ctas: [
      { label: "Kúpiť Univerzál", to: "/produkty#univerzal", primary: true },
      { label: "Celá ponuka", to: "/produkty" },
    ],
  },
  {
    kind: "image",
    ...
    note: "100 % recyklovateľné obaly · vyrobené v EÚ",
    ctas: [
      { label: "Prečo Droply", to: "/ako-funguje", primary: true },
      { label: "Spočítať úsporu", to: "/ako-funguje" },
    ],
  },
];
...
function BadgePill({ badge }: { badge: Badge }) {
  ...
  return (
    <span className={`inline-flex items-center gap-1.5 rounded-full px-3.5 py-1.5 text-xs font-bold uppercase tracking-wide shadow-card ${BADGE_STYLE[badge.tone]}`}>
...
export default function HeroSlider() {
  ...
  return (
    <section id="top" className="relative h-[100svh] min-h-[600px] w-full overflow-hidden bg-secondary" onMouseEnter={stop} onMouseLeave={play}>
          ...
          {SLIDES.map((s, i) => (
            <div key={i} className="relative h-full min-w-0 flex-[0_0_100%]">
              {s.kind === "video" ? (
                ...
                <img src={s.media} alt="" className="absolute inset-0 h-full w-full object-cover object-[72%_center] sm:object-center" />
              )}
                  ...
                  <div className="max-w-2xl">
                    ...
                    <div className="mt-8 flex flex-wrap items-center gap-3">
                      {s.ctas.map((c) => (
                        ...
                        >
                          {c.label}
                          {c.primary && <ArrowUpRight className="h-5 w-5 transition-transform group-hover:translate-x-0.5 group-hover:-translate-y-0.5" />}
                        </Link>
                      ))}
                    </div>
                  </div>
      ...
      <button onClick={prev} className="absolute left-4 top-1/2 z-20 hidden -translate-y-1/2 place-items-center rounded-full border border-white/25 bg-secondary/30 p-3 text-white backdrop-blur transition-colors hover:bg-secondary/60 sm:grid" aria-label="Predchádzajúci">
        <ChevronLeft className="h-5 w-5" />
      </button>
      <button onClick={next} className="absolute right-4 top-1/2 z-20 hidden -translate-y-1/2 place-items-center rounded-full border border-white/25 bg-secondary/30 p-3 text-white backdrop-blur transition-colors hover:bg-secondary/60 sm:grid" aria-label="Ďalší">
      ...
      <div className="absolute bottom-7 left-1/2 z-20 flex -translate-x-1/2 items-center gap-2.5">
        {SLIDES.map((_, i) => (
          <button
            ...
            className={`h-2.5 rounded-full transition-all duration-300 ${i === selected ? "w-8 bg-white" : "w-2.5 bg-white/45 hover:bg-white/70"}`}
          />
        ))}
      </div>
    </section>
